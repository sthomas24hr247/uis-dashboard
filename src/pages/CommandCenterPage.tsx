...
const C = {
  bgDeep: '#060610', textPrimary: '#f0f0f5', textSec: 'rgba(240,240,245,.5)', textTer: 'rgba(240,240,245,.3)',
...
const moneyK = (n: any) => n == null ? '—' : Math.abs(n) >= 1000000 ? '$' + (n / 1000000).toFixed(2) + 'M' : Math.abs(n) >= 1000 ? '$' + (n / 1000).toFixed(1) + 'K' : money(n);
...
export default function CommandCenterPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const [g, p, pr] = await Promise.all([apiFetch('/api/outcome-gap/summary'), apiFetch('/api/predictions/summary'), apiFetch('/api/dashboard/practice-summary')]);
  ...
  const alerts = [
    { key: 'attrition', dot: C.amber, badge: 'WARNING ALERT', title: 'SILENT ATTRITION', subtitle: 'Predictive Patient Churn', confidence: 87,
      ...
      actions: [{ pri: 'HIGH', text: 'Launch targeted reactivation outreach for the 302 critical-risk patients' }, { pri: 'MED', text: 'Prioritize recall scheduling for 269 high-risk patients' }], link: '/patient-intel', linkLabel: 'OPEN PATIENT INTEL' },
    { key: 'leakage', dot: C.pink, badge: 'REVENUE ALERT', title: 'TREATMENT LEAKAGE', subtitle: 'Planned Care Not Completed', confidence: 92,
      ...
      actions: [{ pri: 'HIGH', text: 'Chase the 1,654 actionable episodes — $615K with no blockers' }, { pri: 'MED', text: 'Resolve the 26 queued episodes (missing patient contact info)' }], link: '/outcome-gap/classified', linkLabel: 'OPEN REVENUE RECOVERY' },
    { key: 'recovery', dot: C.green, badge: 'OPPORTUNITY', title: 'HIGH-VALUE RECOVERY', subtitle: 'Immediately Actionable Revenue', confidence: 95,
      ...
      actions: [{ pri: 'HIGH', text: 'Sort actionable episodes by value and begin outreach top-down' }], link: '/outcome-gap/classified', linkLabel: 'OPEN REVENUE RECOVERY' },
  ];
  const A = alerts[selected];

  // Real intelligence feed events (from live data + real run cadence)
  const feed = loading ? [] : [
    { c: C.cyan, text: 'AI predictions refreshed', meta: `${scored?.toLocaleString() ?? '—'} patients scored · model noshow-attr-v1`, when: 'Today 11:00 AM' },
    { c: C.amber, text: `${critical} patients flagged critical churn risk`, meta: 'Attrition model · high-priority reactivation', when: 'Today' },
    { c: C.pink, text: `${atRisk} patients identified at attrition risk`, meta: `of ${scored?.toLocaleString() ?? '—'} scored`, when: 'Today' },
    { c: C.green, text: '1,654 recoverable episodes classified', meta: '$615K actionable, no blockers', when: 'Today' },
    { c: C.purple, text: `${stalledCount?.toLocaleString() ?? '—'} treatment episodes flagged stalled`, meta: `${gap?.overall_gap_pct ?? '—'}% outcome gap`, when: 'Today' },
    { c: C.green, text: 'Dentrix sync completed', meta: 'Practice roster current · 38 providers', when: 'Today 12:55 AM' },
    { c: C.cyan, text: 'Practice data current', meta: `${active?.toLocaleString() ?? '—'} active patients · QCI ${qci ?? '—'}`, when: 'Live' },
  ];
        ...
        <div style={{ position: 'relative', padding: 20 }}>
          <div style={{ border: `1px solid ${C.borderMedium}`, borderRadius: 16, overflow: 'hidden', background: 'rgba(6,6,16,.6)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10, padding: '12px 18px', borderBottom: `1px solid ${C.borderSubtle}`, background: 'rgba(255,255,255,.02)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><Dot c={C.cyan} /><span style={mono({ fontSize: 11, letterSpacing: '.2em', color: C.cyan })}>UIS EXECUTIVE COMMAND CENTER</span><span style={mono({ fontSize: 10, color: C.textTer })}>{practice?.name || 'PoshPearl'} · {practice?.location || 'Yucaipa, CA'}</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, ...mono({ fontSize: 10, letterSpacing: '.1em', color: C.textSec }) }}>
            ...
            {/* 3-column body */}
            <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr 320px', minHeight: 540 }}>
              {/* Alert rail */}
              ...
              {/* Alert detail */}
              <div style={{ padding: 24, borderRight: `1px solid ${C.borderSubtle}` }}>
                ...
                <p style={{ fontSize: 15, lineHeight: 1.7, color: C.textSec, marginTop: 20 }}>{A.narrative}</p>
                <div style={{ marginTop: 20, borderRadius: 12, border: `1px solid rgba(251,113,133,.2)`, background: 'rgba(251,113,133,.05)', padding: 18 }}>
                  <div style={mono({ fontSize: 10, letterSpacing: '.12em', color: C.red })}>◈ {A.revLabel}</div>
                ...
                <div style={{ marginTop: 16, borderRadius: 12, border: `1px solid rgba(125,211,252,.2)`, background: 'rgba(125,211,252,.04)', padding: 18 }}>
                ...
                {loading ? <div style={mono({ fontSize: 11, color: C.textTer, padding: 12 })}>LOADING…</div> : feed.map((f, i) => (
                  <div key={i} style={{ display: 'flex', gap: 10, padding: '11px 0', borderTop: i > 0 ? `1px solid ${C.borderSubtle}` : 'none' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={mono({ fontSize: 9.5, color: C.textTer, marginTop: 3 })}>{f.meta}</div>
                      <div style={mono({ fontSize: 9, color: 'rgba(240,240,245,.22)', marginTop: 3 })}>{f.when}</div>
                    </div>
                  </div>
                ))}
              </div>
